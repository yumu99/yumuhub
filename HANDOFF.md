...
- **MCP onboarding** — `MCP_PRESETS` (in `YumuHub.jsx` just above `McpServersSection`) + one-click "+ preset" buttons (Everything/Memory/Sequential-Thinking/Filesystem, all `npx -y`) pre-fill a server row so connecting a first server is one click + Start. The natural next step the UX-lens subagent flagged.
...
- **MCP follow-ups:** remote/SSE transport (stdio only today). No per-tool approval gating specific to MCP tools. Server-mode (expose yumuHub's *own* tools over MCP) is the reverse direction, still open. (Hardening + one-click presets + onboarding all done — see below.)
